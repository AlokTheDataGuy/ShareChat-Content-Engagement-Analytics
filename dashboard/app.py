...
def _load_ab_data():
    """Fetch all A/B test data from warehouse. Cached so repeated tab visits are instant."""
    conn = get_db_conn()
    if conn is None:
        return None, None
    ab_sum = pd.read_sql(
...
def page_ab_test():
    ...
    # Load data (first visit shows spinner; subsequent visits use cache instantly)
